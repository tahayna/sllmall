...
class ShopRepository extends Repository
{
    ...
    public static function storeByRequest(ShopCreateRequest $request): Shop
    {
        ...
        $certificate = null;
        if ($request->hasFile('shop_registration_certificate')) {
            $certificate = MediaRepository::storeByRequest(
                $request->shop_registration_certificate,
                'shops/registration-certificate',
                'file' // if your MediaRepository supports 'file'
            );
        }
    ...
    'registration_certificate_id' => $certificate ? $certificate->id : null,

    'delivery_charge' => $request->delivery_charge ?? 0,
    'min_order_amount' => $request->min_order_amount ?? 0,
    'prefix' => $request->prefix,
    'opening_time' => $request->opening_time,   // <-- add
    'closing_time' => $request->closing_time,   // <-- add
